...
const resetScrollToCell = async (table, dl, cell = SELECTED_POSITION) => {
    const _dl = dl();
    await table.scrollToCell(_dl.num_columns, _dl.num_rows, _dl.num_columns, _dl.num_rows);
    await table.scrollToCell(cell.x, cell.y, _dl.num_columns, _dl.num_rows);
};

const addKeyboardNavigation = async (table, dl, write, editable = true) => {
