...
macro_rules! js_obj_field {
    ...
    ($value:expr, $key:expr, $def:expr) => {{
        use js_intern::js_intern;
        Reflect::get($value, js_intern!($key)).map(|x| x.unchecked_into::<js_sys::Object>()).unwrap_or($def)
    }};
}

#[macro_export]
macro_rules! js_val_field {
    ($value:expr, $key:expr) => {{
        use js_intern::js_intern;
        Reflect::get($value, js_intern!($key))
    }};

    ($value:expr, $key:expr, $def:expr) => {{
        use js_intern::js_intern;
        Reflect::get($value, js_intern!($key)).unwrap_or($def)
    }};
}
