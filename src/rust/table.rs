    ...
    pub fn draw_row_headers(
        ...
        if !row_headers.is_undefined() && row_headers.length() > 0 {
            ...
            Reflect::set(
                ...
                &cont_heads.iter().enumerate().fold(Ok(JsValue::from(0.0)), |total: Result<JsValue, JsValue>, _obj| {
                    let (i, b) = _obj;
                    let th = js_html_field!(b, "th")?;
                    let _indices = js_arr_field!(&self._column_sizes, "indices")?;
                    let item = _indices.get(i as u32);
                    let width = if item.is_undefined() { th.offset_width() as f64 } else { item.as_f64().unwrap() };
                    Ok(JsValue::from(total?.as_f64().unwrap() + width))
                })?,
            )?;
